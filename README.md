...
- ADMIN의 관리자·조직 승인, 중앙 AI 설정·로그, 초대 정책 관리
- 조직 관리자별 승인 조직 범위 적용, 조직코드 가입요청 승인
- 조직별 시험 생성, 문제 등록, 응시자 등록·일괄 배정·배정 해제
- AI와 대화하며 코딩 문제(제목·예제·숨김 테스트·채점 방식)를 함께 작성하는 문제 생성 챗봇
...
| 역할 | 접근 | 주요 기능 |
| --- | --- | --- |
| ADMIN | 운영자 로그인 | 관리자·조직 승인, 중앙 AI 연동 설정·채점 대기열·로그, 초대 정책 관리 |
| 매니저(MANAGER) | 운영자 로그인 | 조직 운영, 시험·문제·응시자·초대·실시간 관제·결과 관리 |
...
ADMIN이 **시험 조회 및 설정 → 중앙 AI 채점 설정**에서 공급자와 API 키를 등록하면, 모범 답안 생성·자동 채점·문제 생성 챗봇이 같은 연동을 공유합니다. 현재 OpenAI, Anthropic, Google Gemini, DeepSeek을 지원합니다. 등록된 키는 서버에 암호화되어 저장되며, 등록 화면에서 "API 키 확인" 버튼으로 공급자별 실제 API에 대해 즉시 검증할 수 있습니다. 모든 호출은 ADMIN의 **AI 프롬프트·응답 로그** 화면에서 감사 로그로 조회됩니다.
...
| 초대 링크·응시번호 | 구현됨, 시험 종료 시각에 맞춰 링크 만료 |
| 객관식 시험 제출·점수 | 구현됨 |
| AI 에이전트 기반 문제 생성 | 구현됨 (OpenAI/Anthropic/Gemini/DeepSeek 중 등록된 중앙 AI 사용) |
...
상단 내비게이션(`frontend/src/components/Header.jsx`)에 정의된 실제 메뉴 구조입니다.
...
| 시험 운영 | 시험 총괄 대시보드(시험 생성·문제 관리·응시자 배정, AI 챗봇 문제 생성 포함) · 시험 금지사항 관리 |
| 감독·결과 | 화상 모니터링 · 응시자 관리(AI 채점 리포트) |
