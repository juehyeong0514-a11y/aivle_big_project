...
YOLO 기반 신분증 OCR 서비스는 [id-ocr-service/README.md](id-ocr-service/README.md)를 따라 별도 Render Web Service로 배포합니다. `best.pt`는 `id-ocr-service/models/best.pt`에 포함되어 있으며, 기존 백엔드에는 OCR 서비스의 `/ocr/id-card` 주소를 `ID_CARD_OCR_URL`로 등록합니다.
