...
test("updates an exam schedule across invitations and removes the exam graph", async (context) => {
  ...
  assert.equal(renamedExam.title, "Renamed schedule test");
  assert.equal(renamedExam.date, "2099.10.11 12:00");
...
test("governs organization approval, manager scope, and invitations reusable before submission", async (context) => {
  ...
  const firstExamCandidateList = await firstExamCandidates.json();
  const testCandidate = firstExamCandidateList.find((item) => item.isTestCandidate === true);
  assert.ok(testCandidate);
  assert.equal(testCandidate.name, "테스트 응시자");
  assert.equal(testCandidate.email, "manager2@aivle.com");
  assert.deepEqual(firstExamCandidateList.filter((item) => !item.isTestCandidate).map((item) => item.id), [candidate.id]);
  ...
  const secondExamCandidateList = await secondExamCandidates.json();
  assert.equal(secondExamCandidateList.length, 1);
  assert.equal(secondExamCandidateList[0].isTestCandidate, true);
  assert.notEqual(secondExamCandidateList[0].id, testCandidate.id);
  ...
  const testInvitationResponse = await fetch(`${baseUrl}/api/manager/exams/${exam.id}/invitations/send`, { method: "POST", headers: managerHeaders, body: JSON.stringify({ candidateIds: [testCandidate.id] }) });
  assert.equal(testInvitationResponse.status, 201);
  const testInvitation = await testInvitationResponse.json();
  const testToken = new URL(testInvitation.mailPreviews[0].entryLink).searchParams.get("token");
  const testInvitationInfo = await fetch(`${baseUrl}/api/invitations/${testToken}`);
  assert.equal((await testInvitationInfo.json()).skipPrecheck, true);
  const verifiedTestInvitation = await fetch(`${baseUrl}/api/invitations/${testToken}/verify`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ candidateNumber: testCandidate.candidateNumber }) });
  const testApplicant = await verifiedTestInvitation.json();
  assert.equal(testApplicant.skipPrecheck, true);
  const testApplicantSession = await fetch(`${baseUrl}/api/applicant/session`, { headers: { Authorization: `Bearer ${testApplicant.accessToken}` } });
  assert.equal((await testApplicantSession.json()).skipPrecheck, true);
